import { a as y$1, i as h, n as d, r as f } from "./LoaderBase-ATuDWTDB-CVvxHxbW.js";
//#region node_modules/3d-tiles-renderer/build/QuantizedMeshLoaderBase-Cn33qyYc.js
var it = class {
	/**
	...
	refreshToken(t) {
		if (this._tokenRefreshPromise === null) {
			...
			e.searchParams.set("access_token", this.apiToken), this._tokenRefreshPromise = fetch(e, t).then((s) => {
				if (!s.ok) throw new Error(`CesiumIonAuthPlugin: Failed to load data with error code ${s.status}`);
				return s.json();
...
var D = "https://tile.googleapis.com/v1/createSession";
var nt = class {
	...
	refreshToken(t) {
		if (this._tokenRefreshPromise === null) {
			...
			this.isMapTilesSession && (s.method = "POST", s.body = JSON.stringify(this.sessionOptions), s.headers = s.headers || {}, s.headers = {
				...
				if (!i.ok) throw new Error(`GoogleCloudAuth: Failed to load data with error code ${i.status}`);
				return i.json();
...
function N(h$1) {
	if ("session" in h$1) return h$1.session;
	{
		let t = null;
		const e = h$1.root;
		return h(e, (s) => {
...
var rt = "https://tile.googleapis.com/v1/3dtiles/root.json";
var at = class {
	/**
	* @param {Object} options
	* @param {string} options.apiToken
	...
	*/
	constructor({ apiToken: t, sessionOptions: e = null, autoRefreshToken: s = !1, logoUrl: i = null, useRecommendedSettings: o = !0 }) {
		this.name = "GOOGLE_CLOUD_AUTH_PLUGIN", this.apiToken = t, this.useRecommendedSettings = o, this.logoUrl = i, this.auth = new nt({
			apiToken: t,
			autoRefreshToken: s,
			sessionOptions: e
		}), this.tiles = null, this._visibilityChangeCallback = null, this._attributionsManager = new ot(), this._logoAttribution = {
			value: "",
			type: "image",
			collapsible: !1
		}, this._attribution = {
	...
	init(t) {
		...
		t.resetFailedTiles(), t.rootURL ??= rt, s.sessionOptions || (s.authURL = t.rootURL), e && !s.isMapTilesSession && (t.errorTarget = 20), this.tiles = t, this._visibilityChangeCallback = ({ tile: i, visible: o }) => {
			var l, a;
...
var lt = class {
	...
	*/
	constructor(t = {}) {
		const { apiToken: e, assetId: s = null, autoRefreshToken: i = !1, useRecommendedSettings: o = !0, assetTypeHandler: r = (l, a, w) => {
			console.warn(`CesiumIonAuthPlugin: Cesium Ion asset type "${l}" unhandled.`);
		} } = t;
		this.name = "CESIUM_ION_AUTH_PLUGIN", this.auth = new it({
			apiToken: e,
			autoRefreshToken: i
		}), this.assetId = s, this.autoRefreshToken = i, this.useRecommendedSettings = o, this.assetTypeHandler = r, this.tiles = null, this._tileSetVersion = -1, this._attributions = [];
	}
	init(t) {
...
var ct = class extends y$1 {
	constructor(...t) {
		super(...t), this.fetchOptions.header = { Accept: "application/vnd.quantized-mesh,application/octet-stream;q=0.9" };
	}
	loadAsync(...t) {
	...
	parse(t) {
		...
		let C = 0, S = 0, I = 0;
		const U = 32767;
		for (let n = 0; n < c; ++n) C += v($[n]), S += v(q[n]), I += v(J[n]), m[n] = C / U, R[n] = S / U, P[n] = I / U;
		...
		const x = (n, u) => R[u] - R[n], X = (n, u) => -x(n, u), E = (n, u) => m[n] - m[u], Z = (n, u) => -E(n, u), B = a(r(), k);
		B.sort(x);
		const F = a(r(), k);
		F.sort(E);
		const z = a(r(), k);
		...
		for (; e < s.byteLength;) {
			...
			if (n === 1) {
				...
				A.watermask = {
					extensionId: n,
					mask: a(f * f, Uint8Array),
					size: f
				};
			} else if (n === 4) {
				const p = a(r(), Uint8Array), d = new TextDecoder().decode(p);
				A.metadata = {
					extensionId: n,
					json: JSON.parse(d)
				};
...
//#endregion
//#region node_modules/3d-tiles-renderer/build/index.core-plugins.js
function p(u) {
...
function y(u, i) {
	...
	const e = u.implicitTilingData.x, t = u.implicitTilingData.y, r = u.implicitTilingData.z;
	return [
		2 * e + i % 2,
		2 * t + Math.floor(i / 2) % 2,
		p(u) ? 2 * r + Math.floor(i / 4) % 2 : 0
	];
}
var b = class {
	constructor(i, e) {
	...
	static clone(i) {
		return {
			...
			implicitTilingData: { ...i.implicitTilingData }
		};
	}
};
var A = class extends y$1 {
	constructor(i) {
	...
	*/
	parseBuffer(i) {
		...
		t += 8;
		const o = JSON.parse(f(new Uint8Array(i, t, s)));
		t += s;
		return {
			version: n,
			subtreeJson: o,
			subtreeByte: i.slice(t, t + l)
		};
	}
	async parse(i) {
		const e = this.parseBuffer(i), t = e.subtreeJson;
		t.contentAvailabilityHeaders = [].concat(t.contentAvailability);
		const r = this.preprocessBuffers(t.buffers), n = this.preprocessBufferViews(t.bufferViews, r);
		this.markActiveBufferViews(t, n);
		const s = await this.requestActiveBuffers(r, e.subtreeByte), l = this.parseActiveBufferViews(n, s);
	...
	*/
	markActiveBufferViews(i, e) {
		...
		for (let l = 0; l < n.length; l++) t = void 0, isNaN(n[l].bitstream) ? isNaN(n[l].bufferView) || (t = e[n[l].bufferView]) : t = e[n[l].bitstream], t && (t.isActive = !0, t.bufferHeader.isActive = !0);
		t = void 0;
	...
	*/
	async requestActiveBuffers(i, e) {
		...
		for (let s = 0; s < i.length; s++) {
			const l = i[s];
			if (!l.isActive) t.push(Promise.resolve());
			else if (l.isExternal) {
				const o = this.parseImplicitURIBuffer(this.tile, this.rootTile.implicitTiling.subtrees.uri, l.uri), a = fetch(o, this.fetchOptions).then((c) => {
					if (!c.ok) throw new Error(`SUBTREELoader: Failed to load external buffer from ${l.uri} with error code ${c.status}.`);
					return c.arrayBuffer();
				}).then((c) => new Uint8Array(c));
				t.push(a);
			} else t.push(Promise.resolve(new Uint8Array(e)));
		}
	...
	*/
	parseActiveBufferViews(i, e) {
		...
		for (let r = 0; r < i.length; r++) {
			...
			const s = n.byteOffset, l = s + n.byteLength;
			t[r] = e[n.buffer].slice(s, l);
		}
		return t;
	}
	/**
	...
	*/
	parseAvailability(i, e, t) {
		...
		i._tileAvailability = this.parseAvailabilityBitstream(e.tileAvailability, t, s), i._contentAvailabilityBitstreams = [];
		for (let o = 0; o < e.contentAvailabilityHeaders.length; o++) {
			const a = this.parseAvailabilityBitstream(e.contentAvailabilityHeaders[o], t, s);
			i._contentAvailabilityBitstreams.push(a);
		}
		i._childSubtreeAvailability = this.parseAvailabilityBitstream(e.childSubtreeAvailability, t, l);
	}
	/**
	...
	*/
	expandSubtree(i, e) {
		const t = b.clone(i);
		for (let s = 0; e && s < e._contentAvailabilityBitstreams.length; s++) if (e && this.getBit(e._contentAvailabilityBitstreams[s], 0)) {
			t.content = { uri: this.parseImplicitURI(i, this.rootTile.content.uri) };
			break;
		}
		i.children.push(t);
		const r = this.transcodeSubtreeTiles(t, e), n = this.listChildSubtrees(e, r);
		for (let s = 0; s < n.length; s++) {
			const l = n[s], o = l.tile, a = this.deriveChildTile(null, o, null, l.childMortonIndex);
	...
	transcodeSubtreeTiles(i, e) {
		...
		for (let n = 1; n < this.rootTile.implicitTiling.subtreeLevels; n++) {
			...
			for (let a = 0; a < o; a++) {
				...
				const g = this.deriveChildTile(e, f, c, a);
	...
	*/
	deriveChildTile(i, e, t, r) {
		...
		for (let s = 0; i && s < i._contentAvailabilityBitstreams.length; s++) if (i && this.getBit(i._contentAvailabilityBitstreams[s], t)) {
	...
	*/
	getBit(i, e) {
		if (e < 0 || e >= i.lengthBits) throw new Error("Bit index out of bounds.");
		if (i.constant !== void 0) return i.constant;
		const t = e >> 3, r = e % 8;
	...
	getTileBoundingVolume(i) {
		...
		if (this.rootTile.boundingVolume.box) {
			...
			for (let l = 0; l < s; l++) {
				...
				t[0] += 2 * o * (-.5 * r + h), t[1] += 2 * a * (-.5 * r + h), t[2] += 2 * c * (-.5 * r + h);
			}
			e.box = t;
		}
		return e;
...
//#endregion
export { it as CesiumIonAuth, lt as CesiumIonAuthPlugin, x as EnforceNonZeroErrorPlugin, nt as GoogleCloudAuth, at as GoogleCloudAuthPlugin, w as ImplicitTilingPlugin, ct as QuantizedMeshLoaderBase };

//# sourceMappingURL=3d-tiles-renderer_core_plugins.js.map