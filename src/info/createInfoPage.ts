import { flightHref } from "../appRoute";
import { INFO_AIRCRAFT } from "./aircraft";
import "./info.css";

function blobBase(): string {
  const slug = /^[A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+$/.test(__REPOSITORY_SLUG__)
    ? __REPOSITORY_SLUG__
    : "0SFS/0SFS.github.io";
  return `https://github.com/${slug}/blob/main`;
}
...
export function createInfoPage(root: HTMLElement): void {
  const docs = blobBase();
  const repo = docs.replace(/\/blob\/main$/, "");
  ...
  page.innerHTML = `
    ...
    <header class="info-page__hero">
      ...
      <p class="info-page__actions">
        ...
        <a class="info-page__secondary" href="${repo}">Source on GitHub</a>
      </p>
    </header>
    <main id="info-main" class="info-page__main">
      ...
      <section aria-labelledby="info-aircraft">
        ...
        <div class="info-page__aircraft">
          ${INFO_AIRCRAFT.map((aircraft) => `
            <article>
              <img src="${assetUrl(aircraft.thumbnail)}" alt="" width="640" height="360" />
              <h3>${aircraft.label}</h3>
              <p>${aircraft.summary}</p>
              ${aircraft.developmentNote ? `<p class="info-page__note">${aircraft.developmentNote}</p>` : ""}
            </article>
          `).join("")}
        </div>
      ...
      <section aria-labelledby="info-controls">
        ...
        <table>
          ...
          <tbody>
            ${CONTROLS.map(([input, action]) => `<tr><th scope="row">${input}</th><td>${action}</td></tr>`).join("")}
          </tbody>
        </table>
      ...
      <section aria-labelledby="info-docs">
        ...
        <ul>
          <li><a href="${docs}/docs/development.md">Development</a></li>
          <li><a href="${docs}/docs/deploying.md">Deploying to GitHub Pages</a></li>
          <li><a href="${docs}/docs/jsbsim.md">How JSBSim runs in the browser</a></li>
          <li><a href="${docs}/docs/sound.md">Sound</a></li>
          <li><a href="${docs}/docs/foss-earth-relationship.md">FOSS Earth relationship</a></li>
          <li><a href="${docs}/docs/phone-controller.md">Phone controller</a></li>
        </ul>
      </section>
    </main>
  ...
  `;
  const start = page.querySelector(".info-page__start");
  if (start instanceof HTMLAnchorElement) start.setAttribute("href", flightHref(new URL(window.location.href)));
