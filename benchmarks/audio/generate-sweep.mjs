// The fixed telemetry script from docs/sound.md §5.
// Writes the generated fixture under gitignored build/; scripts and the SHA-256
// stay in this directory.
//
//   node benchmarks/audio/generate-sweep.mjs
//   node benchmarks/audio/generate-sweep.mjs --stdout
//
...
export const SWEEP_PATH = "build/benchmarks/audio/sweep.jsonl";

export function generateSweepText() {
...
function main() {
  const text = generateSweepText();
  if (process.argv.includes("--stdout")) {
    process.stdout.write(text);
    return;
  }
  mkdirSync(path.dirname(SWEEP_PATH), { recursive: true });
  writeFileSync(SWEEP_PATH, text);
  process.stderr.write(`wrote ${SWEEP_PATH} (${text.length} bytes)\n`);
}
